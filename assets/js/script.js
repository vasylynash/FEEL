...
function showRecipes() {
  ...
  var url =
    ...
    "?" +
    queryString +
    `&app_id=${APP_ID}&app_key=${APP_KEY}&from=${PAGE_SIZE * currentPage}&to=${
...
function renderSearchResults(data) {
  ...
  for (let i = 0; i < recipes.length; i++) {
    ...
    var resultBody = $(`<div>`);
    var title = $(
      `<h3><a class="btn btn-link" href="${url}" target="_blank" rel="noopener noreferrer" id="recipe-name" value="${name}">${name}</a></h3>`
    );
